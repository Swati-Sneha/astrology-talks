...
from app.database.models.users import User, Login, SuccessfulLoginResponse
...
async def login(request: Request, login: Login) -> JSONResponse:
    try:
        ...
        if user and bcrypt.checkpw(login.password.encode(), user.get("password")):
            payload = {
                ...
                "dob": user.get("dob") if user.get("dob") else None,
